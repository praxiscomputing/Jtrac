...
/**
 * Abstract class that serves as base for both Item and History this contains
 * the fields that are common to both and persisted
 */
...
public abstract class AbstractItem implements Serializable, DocumentCreator {
	...
	public SpaceSeverityPeriod getSpaceSeverityPeriod() {
		return spaceSeverityPeriod;
	}

	public void setSpaceSeverityPeriod(SpaceSeverityPeriod spaceSeverityPeriod) {
	...
	// probably belong to Item not AbstractItem, but convenient for
	// item_view_form binding
	private Set<ItemUser> itemUsers;
	private Set<ItemUser> itemUsersSMS;
	...
	public abstract Severity getSeverityFromSpaceSeverityPeriod();
	...
	public String getCustomValue(Field.Name fieldName) {
		...
		if (fieldName.getType() <= 3) {
			return getSpace().getMetadata().getCustomValue(fieldName,
					(Integer) getValue(fieldName));
		} else {
	...
	public Set<ItemUser> getItemUsersSMS() {
		return itemUsersSMS;
	}

	public void setItemUsersSMS(Set<ItemUser> itemUsersSMS) {
		this.itemUsersSMS = itemUsersSMS;
	}
